...
def extract_name_and_rank(text):
    ...
    first_line = lines[0]
    if re.match(r'^[A-Z][a-z]+\s+[A-Z][a-z]+(?:\s+[A-Z][a-z]+)?
...
def parse_contact_block(contact_text, source_filename):
    """Parse contact block with name/rank separation"""
...
def main():
    ...
    for contact in all_contacts:
        # Create dedup key
        key = (
            contact['email'].replace(' [PLACEHOLDER]', '').lower(),
            contact['name'].lower(),
        ...
        if key not in unique_contacts:
            ...
            # Merge: prefer real email, complete data
    ...
    fieldnames = ['name', 'rank', 'position', 'email', 'phone', 'organization', 'address', 'sector', 'raw_data', 'source_file']
...
def extract_email_addresses(text):
    """Extract email addresses with better placeholder detection"""
    email_pattern = r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b'
    emails = re.findall(email_pattern, text, re.IGNORECASE)
    ...
    is_placeholder = any(pattern.lower() in email for pattern in placeholder_patterns)
    
    if is_placeholder:
        return f"{email} [PLACEHOLDER]"
    else:
        return email
...
def parse_contact_block(contact_text, source_filename):
    """Parse contact block with enhanced extraction"""
    print(f"  Parsing contact block from {source_filename}:")
    ...
    print(f"    Result: {contact['name']} | {contact['position']} | {contact['email']}")
    return contact
