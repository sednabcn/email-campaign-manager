...
def is_valid_docx(filepath: str) -> Tuple[bool, Optional[str]]:
    """Check if DOCX file is a valid zip archive containing required Word parts."""
    try:
...
def load_campaign_content(campaign_path: str) -> Optional[object]:
    """Universal loader for .docx/.txt/.md/.html/.json campaign templates."""
    ...
    try:
        if ext == ".docx":
            ...
            return load_json_campaign(str(p))

        else:
            print(f"[templates] Unsupported extension: {ext}")
            return None
    except Exception as e:
        print(f"[templates] Load error for {p}: {e}")
        traceback.print_exc()
        return None
...
# ==============================================================
# Utility functions (contacts, tracking)
# ==============================================================
...
def campaign_main(contacts_root: str, scheduled_root: str, tracking_root: str, alerts_email: str,
    ...
    templates = list(Path(scheduled_root).rglob("*.txt")) + list(Path(scheduled_root).rglob("*.docx")) + list(Path(scheduled_root).rglob("*.json"))
